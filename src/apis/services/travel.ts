...
export const travelService = {
  ...
  updateTravelInfo: async (
    travelId: number,
    travel: NewTravelInfo,
    token: string
  ) => {
    const newPhotos = travel.photos.filter(
      ({ url }) => new URL(url).protocol === 'blob:'
    );
    const oldPhotos = travel.photos.filter(
      ({ url }) => new URL(url).protocol !== 'blob:'
    );
    if (newPhotos.length === 0) {
      return http.patch<{ status: number; message: string }>(
        ...
        {
          title: travel.title,
          photoIds: travel.photos.map(({ id }) => id),
          representativePhotoId: travel.thumbnailPhotoId,
        },
        token
      );
    }
    const metadataArray = newPhotos.map(({ date, GPSCoordinates, file }) => {
      return {
        takenAt: date,
        longitude: GPSCoordinates?.longitude ?? null,
        latitude: GPSCoordinates?.latitude ?? null,
    ...
    // 현재는 브라우저에서 만든 ID로 대표 이미지를 지정하고 있음, 근데 여행을 수정할 때는 업로드한 사진의 ID 필요
    // 사진 파일의 이름과 찍은 시간이 같으면 같은 대표 이미지로 취급하여 ID 선택
    if (travel.thumbnailPhotoId === null) {
      throw new Error('대표 이미지 정보가 없습니다.');
    }
    const uploadedData = await photoService.uploadPhoto(
      newPhotos.map(({ file }) => file),
      metadataArray,
      token
    );
    if (uploadedData.data === undefined) {
    ...
    return http.patch<{ status: number; message: string }>(
      `/api/travels/${travelId}`,
      {
        title: travel.title,
        photoIds: photoIdArray,
        representativePhotoId: uploadedRepresentativePhoto
          ? uploadedRepresentativePhoto.id
          : localRepresentativePhoto?.id,
      },
      token
    );
  },
