...
export default function TravelDiaryPage() {
  ...
  const { data: dayTravel, request: fetchDayTravel } = useApi(
    travelService.getTravelDay
  );

  const { request: createLog, loading: createLogLoading } = useApi(
    travelService.createNewTravelLog
  );
